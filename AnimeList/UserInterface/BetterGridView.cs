...
{
    public partial class BetterGridView : UserControl
    {
        ...
        public BetterGridView(List<IGridViewCell> buttons)
            : base()
        {
            DoubleBuffered = true;
        ...
        public BetterGridView(IGridViewCell[] buttons)
            : this()
        {
        ...
        private void RefreshButtons()
        {
            ...
            // set each button's size, remove buttons that have been removed, add buttons that are new
            var controlsToRemove = new List<UserControl>();
            Controls.Each(_ =>
            {
                var control = _ as UserControl;
                var cell = _ as IGridViewCell;

                // the control is old, so remove it
                if (!_buttons.Contains(cell))
                {
                ...
                control.Size = new Size((int)_buttonWidth, (int)_buttonHeight);
            });
            ...
            _buttons.ForEach(_ =>
            {
                ...
                RefreshCellPosition(control, cellIndex);
        ...
        {
            var X = i % _columns * (ButtonWidth + ButtonMargin * 2) + ButtonMargin;
            var Y = i / _columns * (ButtonHeight + ButtonMargin) + ButtonMargin;
